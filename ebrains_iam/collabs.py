...
class Collab:
    ...
    def from_json(cls, json_obj: dict):
        return cls(**{camel_to_snake(key): value for key, value in json_obj.items()})
    ...
    def list_teams(self, role: TYPE_ROLE, *, token: str):
        ...
        resp = requests.get(
            f"{self.base_url}/team/{role}", headers={"Authorization": f"bearer {token}"}
        )
        resp.raise_for_status()
        users = resp.json().get("users", [])
        return {"users": [User.from_json(user) for user in users]}
    ...
    def add_team(
        ...
        *,
        ...
        resp = requests.put(
            f"{self.base_url}/team/{role}/users/{username}",
            headers={"Authorization": f"bearer {token}"},
        )
        resp.raise_for_status()
        print(
            f"Adding user {username} to collab {self.name} as role {role} successful!",
            file=sys.stderr,
        )
    ...
    def remove_team(
        ...
        *,
        ...
        resp.raise_for_status()
        print(
            f"Deleting user {username} from collab {self.name} as role {role} successful!",
            file=sys.stderr,
        )
...
def get_collab(collab_id: str, *, token: str):
    resp = requests.get(
        f"{wiki_endpoint}/rest/v1/collabs/{collab_id}",
        headers={"Authorization": f"bearer {token}"},
    )
    resp.raise_for_status()
...
def finditer_collab(*, token: str):
    limit = 50
    offset = 0
    while True:
        resp = requests.get(
            f"{wiki_endpoint}/rest/v1/collabs?limit={limit}&offset={offset}",
            headers={"Authorization": f"bearer {token}"},
        )
        resp.raise_for_status()
        data = resp.json()
        if (len(data)) == 0:
            break
        yield from [Collab.from_json(datum) for datum in data]
        offset += limit
